...
import { AddBookToListInterface, RemoveBookInterface, SearchBookFilter } from "./book.interface";

export const bookApi: any = createApi({
    baseQuery: protectedBaseQuery,
    reducerPath: "bookApi",
    tagTypes: ["book"],
    endpoints: (build) => ({
        getSearchedBooks: build.query({
            query: ({ search, optionValue }: SearchBookFilter) => ({
                url: endpoints.book.searchBookList(search, optionValue),
                method: "GET",
            }),
        }),
        ...
        removeSavedBookFromList: build.mutation({
            query: ({ id, book_id, payload }: { id: string, book_id: string, payload: RemoveBookInterface }) => ({
...
export const { useGetSearchedBooksQuery, useGetBookRecommendationsQuery, useGetBooksQuery, useGetBookByIdQuery, useAddBookToListMutation, useRemoveSavedBookFromListMutation, useGetSavedBooksQuery } = bookApi;
